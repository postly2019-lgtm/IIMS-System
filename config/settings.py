...
# DB readiness configuration
DB_READINESS_MAX_RETRIES = int(os.environ.get('DB_READINESS_MAX_RETRIES', '5'))
DB_READINESS_BACKOFF_BASE = float(os.environ.get('DB_READINESS_BACKOFF_BASE', '0.5'))

# Safe DATABASE_URL logging
_db_url_raw = os.environ.get('DATABASE_URL', '')
_startup_logger = logging.getLogger('iims.startup')
...
if _db_url_raw:
    _startup_logger.info(f"DATABASE_URL present: {_mask_db_url(_db_url_raw)}")
else:
    _startup_logger.warning('DATABASE_URL missing; using default configuration')
...
if ALLOWED_HOSTS_ENV:
    ...
    if DEBUG or IS_TESTING:
        ...
        for var_name in ('RAILWAY_PUBLIC_DOMAIN', 'RAILWAY_PRIVATE_DOMAIN', 'RAILWAY_STATIC_URL', 'STAGING_RAILWAY_HOST'):
...
if DEBUG:
    ...
    # Staging Railway host
    staging_host = os.environ.get('STAGING_RAILWAY_HOST', '').strip()
    if staging_host:
        if not staging_host.startswith('http'):
            staging_host = f'https://{staging_host}'
        if staging_host not in CSRF_TRUSTED_ORIGINS:
            CSRF_TRUSTED_ORIGINS.append(staging_host)
...
DATABASES = {
    'default': dj_database_url.config(
        default=('sqlite:///tmp/db.sqlite3' if os.name != 'nt' else 'sqlite:///db.sqlite3'),
        conn_max_age=600
    )
}
